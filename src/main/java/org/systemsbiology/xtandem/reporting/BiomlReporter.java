        ...
        try {
            ...
            if (isProteinsShown()) {
                ...
                if (proteinPositions.length > 0) {
                    ...
                    String url = getParameters().getDatabaseName() + ".fasta";
                    parentProtein = Protein.getProtein(id, annotation, "", url);
                }

            }
            indent(out, indent);
            out.append("<group ");
            out.append("id=\"" + raw.getIndex() + "\" ");
            ...
            double f = 0;
            if (expected != 0)
                f = Math.log10(expected);
            out.append("expect=\"" + XTandemUtilities.formatDouble(f, 1) + "\" ");
            String ident = getIndexedId(scan, 1);
            out.append("id=\"" + ident + "\" ");
            ...
            if (parentProtein != null) {
                boolean onTwoLines = false;
                writeDescriptionNote(parentProtein.getAnnotation(), out, indent, onTwoLines);
            }
            ...
            if (parentProtein != null) {
                String url = parentProtein.getURL();
                out.append("URL=\"" + url + "\" ");
            }
            out.append(" />");
            out.append("\n");
            ...
            out.append("start=\"1\" ");
            if (parentProtein != null) {
                int sequenceLength = parentProtein.getSequenceLength();
                if (sequenceLength == 0)
                    sequenceLength = polypeptide.getSequenceLength();
                out.append("end=\"" + sequenceLength + "\" ");
            }
            else {
...
//        out.append("</group>");
    ...
    protected void outputHistogram(IScoredScan scan, Appendable out, int indent) {
        try {
            ...
            if (title == null)
                title = ph.getParameter("spectrum, path");
            //     writeDescriptionNote(title + " scan " + scan.getId() + " (charge " + scan.getCharge() + ")", out, indent, false);
            writeDescriptionNote(scan.getId(), out, indent, false);
            ...
            out.append(
                    "<GAML:trace id=\"" + scan.getIndex() + "\" label=\"" + getIndexLabel(scan) + "\" type=\"tandem mass spectrum\">");
            out.append("\n");
    ...
    public static String getIndexedId(IScoredScan scan, int level) {
        StringBuilder sb = new StringBuilder();
        sb.append(Integer.toString(scan.getIndex()));
    ...
    public static String getIndexLabel(IScoredScan scan) {
        return Integer.toString(scan.getIndex()) + ".spectrum";
    }
    ...
    private void outputSpectrumData(final Appendable out, final int indent, IScoredScan scan) {
        ...
        try {
            ...
            out.append("label=\"" + getIndexLabel(scan) + "\" ");
...
//            }
    ...
    protected void outputDomain(IScoredScan scan, Appendable out, int indent) {
        try {
            ...
            indent(out, indent);
            out.append("<domain ");
            out.append("id=\"" + getIndexedId(scan, 2) + "\" ");
