...
import com.lordjoe.distributed.hydra.comet.CometScoringResult;
 ...
 * a Copy of  SparkCometScanScorer set to use dataseta
 * User: Steve
 * Date: 10/7/2014
 */
public class DatasetCometScanScorer {
    ...
    public static void setMaxBinSpectra(int maxBinSpectra) {
        DatasetCometScanScorer.maxBinSpectra = maxBinSpectra;
    }
...
//        System.err.println("Number Spectra " + Long_Formatter.format(numberSpectra) + " Number Peptides " +   Long_Formatter.format(numberpeptides));
        ...
        /**
         * Now we have peptides and spectra - lets generate dataframes
         */
...
//        }
        ...
        GroupedDataset<Row, CometScoringResult> byId = scoreSet.groupBy(id);

        Dataset<Tuple2<Row, CometScoringResult>> reduce = byId.reduce(new AbstractLoggingReduceFunction<CometScoringResult>() {
            @Override
            public CometScoringResult doCall(CometScoringResult r1, CometScoringResult r2) throws Exception {
               r1.addTo(r2);
                return r1;
            }
        });

        Dataset< CometScoringResult> scores = reduce.map(new Tuple2CometScoringResultAbstractLoggingFunction1(),evidence) ;
        ...
        JavaRDD<CometScoringResult> cometScoringResultJavaRDD = fakeRDD.wrapRDD(scores.rdd());

        JavaRDD<IScoredScan> map = cometScoringResultJavaRDD.map(new AbstractLoggingFunction<CometScoringResult, IScoredScan>() {
            @Override
            public IScoredScan doCall(CometScoringResult v1) {
                return v1;
            }
        });


        int numberScores = consolidator.writeScores(map);
        System.out.println("Total Scans Scored " + numberScores);
...
//    }
    ...
    private static class Tuple2CometScoringResultAbstractLoggingFunction1 extends AbstractLoggingFunction1<Tuple2<Row, CometScoringResult>, CometScoringResult> {
        @Override
        public CometScoringResult doApply(Tuple2<Row, CometScoringResult> v1) {
            return v1._2();
        }
     }
}
