...
import com.lordjoe.distributed.input.NullRecordReader;
...
/**
 * com.lordjoe.distributed.hydra.scoring.MZXMLSpectrumInputFormat
 ...
 */
public class MZXMLSpectrumInputFormat extends FileInputFormat<String, IMeasuredSpectrum> {
    ...
    public MZXMLSpectrumInputFormat() {
        m_BaseTag = "scan";
        m_StartTag = "<" + m_BaseTag;
        m_EndTag = "</" + m_BaseTag + ">";
    ...
    @Override
    public RecordReader<String, IMeasuredSpectrum> createRecordReader(InputSplit split,
                                                           TaskAttemptContext context) {
        if (isSplitReadable(split))
            return new MyXMLFileReader(Integer.MAX_VALUE);
        else
            return new NullRecordReader(); // do not read
    }
    ...
    public static long getTotalRecordsOverall() {
        return totalRecordsOverall;
    }

    public static long incrementTotalRecordsOverall(final long pTotalRecordsOverall) {
        totalRecordsOverall += pTotalRecordsOverall;
        return totalRecordsOverall;
    }
     ...
     */
    public class MyXMLFileReader extends RecordReader<String, IMeasuredSpectrum> {
        ...
        private IMeasuredSpectrum m_Value = null;
        ...
        protected void prepareFalseReturn() {
            m_Key = null;
            m_Value = null;
            m_Sb.setLength(0);
            long currentTotal = incrementTotalRecordsOverall(totalRecords);
        }
        ...
        protected boolean readFromCurrentBuffer() {
            ...
            index += endTag.length();
            String scanText = s.substring(0, index).trim();
          // keep the remaining text to add to the next tag
            ...
            try {
                ...
                m_Value = scan;
                // all this is to figure out why we keep doing this twice
                totalRecords++;
                registerIdAndLocation(scan.getId(),m_Current);
                return true;
            }
            catch (Exception e) {
                return false; // need more read
        ...
        @Override
        public IMeasuredSpectrum getCurrentValue() {
            return m_Value;
        }
